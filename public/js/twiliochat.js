var twiliochat = (function() {
  ...
  var GENERAL_CHANNEL_UNIQUE_NAME = $('#channelname').val();
  var GENERAL_CHANNEL_NAME = $('#channelname').val();
  ...
  tc.handleNewChannelInputKeypress = function(event) {
    if (event.keyCode === 13) {
      ...
      //myChannel.sendMessage($newChannelInput.val());
  ...
  function connectClientWithUsername() {
    ...
    fetchAccessToken(tc.username,token, connectMessagingClient);
  }

  function fetchAccessToken(username,token,handler) {
    $.post('http://bizmeth.co.in/BI/token', {identity: username,"_token": token}, null, 'json')
  ...
  tc.addMessageToList = function(message) {
    ...
    if (message.author !== tc.username) {
      rowDiv.addClass('another-message'); 
    }
  ...
  tc.sortChannelsByName = function(channels) {
    return channels.sort(function(a, b) {
      if(GENERAL_CHANNEL_NAME ==  a && GENERAL_CHANNEL_NAME ==  b){
